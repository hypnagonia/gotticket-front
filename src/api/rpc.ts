 ...
 */
export const rpcBigIntAdapter = <T = any>(
  ...
  return fetch
    ...
    .then((res) => res.text())
    .then((res) =>
      JSONBig({ storeAsString: true }).parse(res)
...
export const getAllBalance = (params: [string, 'latest']) => {
  ...
  return Promise.all(
    config.shardUrls.map((shardUrl) => getBalanceHandler(shardUrl))
  );
};
...
export const hmyv2_getTransactionsHistory = (
  params: IGetTxsHistoryParams[]
) => {
  return rpcAdapter<TRPCResponse<{ transactions: RPCTransactionHarmony[] }>>(
    API_URL,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export const hmyv2_getTransactionsCount = (
  address: string,
...
export const hmyv2_getStakingTransactionsHistory = (
  params: IGetTxsHistoryParams[]
) => {
  return rpcAdapter<
    TRPCResponse<{ staking_transactions: RPCTransactionHarmony[] }>
  >(API_URL, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
...
export const hmyv2_getStakingTransactionsCount = (
  address: string,
  txType: RequestTxType = RequestTxType.ALL
) => {
  return rpcAdapter<TRPCResponse<number>>(API_URL, {
...
const mapStakingDelegation = (
  delegation: StakingDelegationResponse
): StakingDelegation => {
  return {
...
export const hmy_getDelegationsByDelegator = (
  address: string
...
export const getAllApprovalsForTokens = async (
  ...
  const contractHarmonyAddr =
    contractAddress && contractAddress.length > 0
      ? new HarmonyAddress(contractAddress)
      : null;

  let dataObj: RPCTransactionHarmony[] = await hmyv2_getTransactionsHistory(
    params
  );
  ...
  for (let tx of dataObj) {
    if (
      ...
      (tx.to === contractAddress ||
        tx.to === contractHarmonyAddr?.bech32 ||
        !contractHarmonyAddr)
    ) {
