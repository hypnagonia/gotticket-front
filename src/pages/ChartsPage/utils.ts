...
export const getDetailedChartOptions = (
  ...
  const ticksColor =
    themeMode === 'light' ? palette.DarkGray : palette.WhiteGrey;
  const tooltipColor = themeMode === 'light' ? '#3f4850' : palette.WhiteGrey;
  const tooltipBorderColor =
    themeMode === 'light' ? '#3f4850' : palette.DarkBlue;
  const tooltipBackground =
    themeMode === 'light'
      ? 'rgba(244, 247, 249, 0.85)'
      : 'rgba(27, 41, 94, 0.95)';
        ...
        ticks: {
          ...
          callback: function (value: string, index: any, ticks: any) {
            ...
            if (nextItem) {
              ...
              if (
                dayjs(item.timestamp).month() !==
                  dayjs(nextItem.timestamp).month() &&
              ...
              if (ticks.length <= 30) {
                // show each day
                if (
                  dayjs(item.timestamp).day() !==
                    dayjs(nextItem.timestamp).day() &&
...
export const downloadMetricsCSV = (
  filename: string,
  params: { items: MetricsDailyItem[] }
) => {
  const { items } = params;
  ...
  const header = mappedTxs
    .filter((_, index) => index === 0)
    .map((item) => Object.keys(item));
  const body = mappedTxs.map((item) => Object.values(item));
